...
6. Format every new commit subject as `<type>(<scope>): <imperative subject>`.
    - Example: `feat(api): add meter-reading endpoint`
    - Example: `bugfix(parser): handle empty input`
    - Example: `chores(ci): align lint configuration`
    - Use a short scope that describes the changed area; never use a scope-before-type form.
